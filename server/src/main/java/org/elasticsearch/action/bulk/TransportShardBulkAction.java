...
public class TransportShardBulkAction extends TransportWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    protected void dispatchedShardOperationOnPrimary(BulkShardRequest request, IndexShard primary,
        ...
        performOnPrimary(request, primary, updateHelper, threadPool::absoluteTimeInMillis,
            (update, shardId, type, mappingListener) -> {
                assert update != null;
                assert shardId != null;
                mappingUpdatedAction.updateMappingOnMaster(shardId.getIndex(), type, update, mappingListener);//更新mapping的方法，如果有新的字段会触发更新(dynamic mapping)
            },
    ...
    public static void performOnPrimary(
        ...
        new ActionRunnable<PrimaryResult<BulkShardRequest, BulkShardResponse>>(listener) {
            ...
            protected void doRun() throws Exception {
                while (context.hasMoreOperationsToExecute()) {//遍历所有bulk items中的request
    ...
    /**
     * Executes bulk item requests and handles request execution exceptions.使用write线程池，被循环调用
    ...
    static boolean executeBulkItemRequest(BulkPrimaryExecutionContext context, UpdateHelper updateHelper, LongSupplier nowInMillisSupplier,
        ...
        if (isDelete) {
            ...
            result = primary.applyIndexOperationOnPrimary(version, request.versionType(), new SourceToParse(// 开始向lucene写入索引结构，里面会去检查mapping是否有更新
        ...
        if (result.getResultType() == Engine.Result.Type.MAPPING_UPDATE_REQUIRED) {// 走进来说明需要进行更新mapping的操作，后面会触发mapping的更新
            ...
            onComplete(result, context, updateResult);// index执行完成
        }
        return true;
    }
    ...
    private static void onComplete(Engine.Result r, BulkPrimaryExecutionContext context, UpdateHelper.Result updateResult) {
        context.markOperationAsExecuted(r);// 标记index操作已执行
    ...
    public static Translog.Location performOnReplica(BulkShardRequest request, IndexShard replica) throws Exception {
        ...
        for (int i = 0; i < request.items().length; i++) {
            ...
            final Engine.Result operationResult;//因为bulk可能部分失败所以要判断一下
